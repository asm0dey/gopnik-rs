...
| 4 | `Game::sell_offer(rng, roll_site, base, span, lines)`; the refund is returned WITH the answer, so a caller cannot roll only on acceptance | delta asserted `== base + the drawn value`, with `base`, `span` and the roll site taken from `data/shop_arms.json`'s `sell.arms[].roll` rather than restated in the test (mutation cases `sell-arm-immediates-come-from-the-artifact`, `all-six-sell-credits-come-from-the-artifact`); `declining_every_wes_offer_still_spends_all_six_draws` and `a_single_declined_offer_spends_its_draw_and_suppresses_the_tail_line` pin the decline half |
