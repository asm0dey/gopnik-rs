...
**Named by Task 33: it is the club card game's STAKE.** `docs/re/club.md`
maps `1000:e065`..`1000:e274`, the `p` arm of `kl`, and
`data/club_arms.json`'s `club.stake_finding` carries the census that makes it
club-local: `python3 tools/re_query.py xrefs-to 20ae:3c82` returns all
fourteen references between `1000:e020` and `1000:e25d` and nowhere else. The
paragraph above stands as written -- every instruction it lists is still what
the bytes say -- but "what the byte prices" is no longer open.
...
Unknown still means unknown. `20ae:3c82` was in that list until Task 33; the
generated artifact still carries `what: null` for file `0xf978` because it has
not been regenerated, and `docs/re/club.md` is the authority on the name in the
meantime.
...
column any more -- `grep -c 'self[.]gate_open(' src/game.rs` returns **1**, the
menu filter, where the same pattern at `fef8c9c` returns 2. The trailing `(` is
what makes that a check: the looser `grep -c 'self.gate_open'` returns 2 at
both revisions, because a doc comment naming the grep took the deleted call's
place. `docs/re/shop-arms.md` has the measurement and the mutations that were
observed red.
