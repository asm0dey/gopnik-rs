...
**`data/other_price_sites.json` records 12 further price sites** (Клуб rows at
15 and 22, a computed `district*50` save charge at `1000:761d`, a `var`-read
BSS site at `20ae:3c82` debited at `1000:e0a3`, and 9 still unidentified
`imm8` sites). Task 11 should start there rather than rediscovering them. Now
generated by `tools/extract_tables.py` like every other `data/*.json`, except
its `computed_sites` entry (the `district*50` row), which has no fixed byte
idiom to scan for and is hand-verified instead.
