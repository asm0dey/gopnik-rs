...
impl Game {
    ...
    pub fn district_advance(
        ...
        self.market_ban_countdown = 0;
        self.club_ban_countdown = 0;
    ...
    fn visit_girl(&mut self) {
        ...
        self.player.money = self.player.money.wrapping_sub(12_i16);
        self.market_ban_countdown = 0;
    }
    ...
    fn row_fill_values(row: &data::ShopEntry) -> Vec<i64> {
        ...
        match (row.shop, row.key) {
            ("mar", "2") => values.push(5), // File 0xD32A
            ("bmar", "7") => values.extend([20, 30]),
            _ => {}
        }
        values
    }
    ...
    pub(crate) fn luck_below_random_32(luck: u16, random: u16) -> bool {
        ...
        if luck_high != random_high {
            return luck_high < random_high; // Signed
        }
        luck < random // Unsigned
    }
    ...
    fn sell_items(
        ...
        if self.wear_suit_abibas && self.wear_suit_adidas {
            ...
            if yes {
                self.wear_suit_abibas = false;
                self.player.money = self.player.money.wrapping_add(refund as i16);
                term::println(&text::fill(
            ...
            "5" => {
                ...
                self.buy_after_gates(
                    ...
                    |g| {
                        ...
                        g.player.dmg_min += 2; // Add word [0x38a8],0x2
                        g.player.dmg_max += 2; // Add word [0x38aa],0x2
    ...
    fn buy_market_row(&mut self, key: &str, price: i32) -> bool {
        match key {
            ...
            "7" => {
                // Intentional oddity, reproduced rather than fixed: there is
                // no district gate on this row, though the menu hides it
                // below district 2 -- so the game sells the adidas suit at
                // district 1 off a menu that never lists it there. There is
                // no better-item gate either.
                let owned = self.wear_suit_adidas;
                // The already-owned check reads a different flag than the
                // one this arm writes, so buying it here never marks itself
                // as already bought.
            ...
            // Row 8, Понтовёйшие бутсы.
            "8" => {
                // This row's district gate is silent -- it skips the row with
                // no message, like row 6's.
                let below_district = self.district <= 2;
                // No better-item gate on this row.
            ...
            // Row 9, Ваще крутую кожанку.
            "9" => {
                // This district gate is silent too, like rows 6 and 8.
    ...
    /// `^0Битва\` is the fight prompt, and it is modal: an unrecognized command (for example `mar`
    /// or `i`) is ignored and the prompt reprints.
    ///
    /// ## The verb set
    ///
    /// Ten commands work at the fight prompt: nine compared against the typed line directly, plus
    /// `h`/`mh`, which goes through a separate call.
    ///
    /// ## Nine independent `if`s, not an `if`/`else` chain
    ///
    /// One `Битва\` prompt runs the whole chain top to bottom, and no arm returns early to the
    /// prompt -- every arm rejoins the same line with the buffer still holding what was typed. Two
    /// consequences follow:
    /// * That is why there are two `k` compares: attacking gives the attack verb a second effect,
    ///   ticking the backup countdown, the next time the same line is checked.
    /// * The backup's own attack block sits between the `v` arm and the `f` compare and belongs to
    ///   neither, so it runs on **every** prompt -- including one whose line matched no compare at
    ///   all.
    ///
    /// Every arm is now implemented. What each one does, in chain order:
    ///
    /// | verb | effect |
    /// |---|---|
    /// | `k` | attack -- [`Game::combat_round`] |
    /// | `run` | flee -- [`Game::flee`] |
    /// | `h`/`mh` | drink -- [`Game::beer`] |
    /// | `kos` | smoke -- [`Game::smoke`] |
    /// | `s` | show your own stats -- [`Game::show_stats`] |
    /// | `sv` | show the enemy's stats -- [`Game::print_enemy_block`] |
    /// | `e` | quit the game immediately -- `Halt(0)` |
    /// | `k` (2nd effect) | ticks the backup countdown -- [`crate::combat_dispatch::Backup::tick_on_attack`] |
    /// | `v` | call in backup -- [`Game::backup_in_fight`] |
    /// | (always) | the backup's own attack -- [`Game::backup_attacks`] |
    /// | `f` | shoot -- [`Game::shoot_in_fight`] |
    ///
    /// Death and victory are both handled at the end of the same fight loop:
    ///
    /// * On death (hp <= 0): fighting the rector is fatal outright, with no rescue
    ///   ([`Game::rector_showdown`]); otherwise, if the den is known and street cred is at least
    ///   10, the hospital rescue fires ([`Game::hospital_rescue`]); the plain case prints `^4Ты
    ///   сдох.` and ends the game entirely, not just the fight.
    /// * On victory: prints `^2Враг сдох.`, then `^6За отпин врага ты получаешь` ...
    ///   (str+agi+vit+luck as the reward). "^2Ты победил." is not printed here -- it is the
    ///   end-of-game banner shown only after beating the rector.
    ///
    /// ## `run` -- fleeing
    ///
    /// This `run` check is combat's own, separate from the general command parser (where `w` and
    /// `run` fold into one Walk command).
    ///
    /// [`Game::flee`] runs the escape, at a cost of [`Game::flee_penalty`] of one level. Fleeing is
    /// refused once the rector showdown has started.
    ///
    /// Fleeing does not end the prompt immediately -- it only sets a flag, and the death and
    /// victory checks still run afterward. So typing `run` in the turn the backup lands the killing
    /// blow counts as a victory, not an escape.
    ///
    /// Fleeing never consumes a random draw.
    ///
    /// ## `opponent_kind` and its five effects
    ///
    /// `opponent_kind` selects the fight context: 0 for a street encounter, 6 for the den, 5 for a
    /// den job, 2 for the club, and 3/4 for the two rector fights ([`Game::rector_endgame`]).
    ///
    /// * Kinds 1, 3 and 4 get their own opening text instead of the class-keyed greeting
    ///   (`crate::ending::OPENER_1` and its two siblings).
    /// * Kind 4 is the victory ending: it runs the marquee and end screen and never returns to the
    ///   ordinary post-fight tail.
    /// * Kind 3 looks like an ending but is a fake-out -- it falls through into the ordinary
    ///   post-fight tail.
    /// * Kinds 3 and 4 skip the XP award and the "too weak an opponent" message.
    /// * Kind 6 gets a понтовость award and an xp award (below); the xp add can spend random draws
    ///   by triggering a capped level-up.
    ///
    /// So any kind other than 0 or 6 skips the ordinary class-keyed greeting and instead takes
    /// whichever of the openers/endings above applies.
    pub(crate) fn run_combat(
        ...
        // The opener only runs for opponent_kind 0 or 6; everything else
        // skips it entirely, which is why the den's cop fight (kind 5) and
        // the club's (kind 2) are silent here. `crate::combat_opener` is the
        // arm.
        if matches!(opponent_kind, 0 | 6) {
            // The rank is passed as a thunk, not a String: only some openers
            // read it, and `rank_name` panics on a class it has no row for,
            // so evaluating it here would widen the panic surface
            // unnecessarily. See `combat_opener::greet`.
            let player_class = self.player.class;
            combat_opener::greet(enemy.class, &self.player.name, || {
                Self::rank_name(player_class)
            });
        }
        // The three other openers all rejoin the same point afterward.
        match opponent_kind {
            // Opponent kind 1 is the market pickpocket's opener: one line
            // and no pause for a keypress. Its only caller is the market's
            // `t` (bust) verb.
            1 => term::println(ending::OPENER_1[0]),
            // Opponent kind 3 is the first rector fight's opener: four
            // lines, each followed by a pause for a keypress.
            3 => {
            ...
            // Opponent kind 4 is the second rector fight's opener: same
            // shape, four lines each with a keypress.
            4 => {
        ...
        // This counter resets once per fight, not once per session.
        let mut prompts_seen: u8 = 0;
        // This is fight-local too, even though it lives in a global
        // variable: it is cleared as the fight function returns, and nothing
        // outside that function ever references it.
        ...
        loop {
            ...
            let line = line?;
            let cmd = parse(&line);
            // The exit flag can be set three ways: fleeing sets it while
            // still inside the compare chain; death and victory set it
            // afterward, when this port processes them after the loop.
            let mut fled = false;

            // The attack arm rejoins the chain afterward, so everything below
            // it still runs in the same turn.
            if cmd == Command::Fight {
                self.combat_round(&mut enemy);
            }

            // This `run` check is combat's own, separate from the general
            // command parser (which folds `w` and `run` into one Walk
            // command).
            ...
            match cmd {
                ...
                // This shows the player's own character sheet.
                Command::Stats => self.show_stats(),
                // This shows the ENEMY's sheet -- a genuinely different
                // screen from the player's own stats shown by `s`.
            ...
            // The second `k` effect only ticks once the backup has actually
            // been called in; it also checks the countdown against a limit of
            // 3, which is what `tick_on_attack` returns.
            if backup.count() >= 1 && cmd == Command::Fight && backup.tick_on_attack() {
                // Intentional: this copy of the line has no trailing dot.
            ...
            // The backup's own attack is not tied to the `v` command -- it
            // runs on every prompt regardless of what was typed, even a line
            // that matched nothing. Both fighters' hp are carried with extra
            // headroom through this step, for the same reason the main attack
            // calc does; only the stored value is clamped.
            let mut ehp = i32::from(enemy.hp);
            self.backup_attacks(&mut backup, &mut ehp, &enemy);

            // Shooting has no enemy-alive check, unlike the backup's own
            // attack: a shot fired in the same prompt the backup lands the
            // killing blow still lands, and its `У него осталось #` can show
            // a negative number.
            ...
            // Nothing else is compared here -- the ten verbs above are the
            // whole in-combat table. The typed-line buffer is shared with
            // other prompts elsewhere in the game (the dealers' menu
            // compares its own verbs against the same buffer), but combat
            // recognizes only its own ten, so a street verb typed at
            // `^0Битва\` reaches no handler at all.
        ...
        // The kind-4 victory ending grants no spoils, no XP, and no item
        // roll -- it runs the marquee and halts without returning to the
        // ordinary post-fight tail.
        if opponent_kind == 4 {
            // Forces a level-up with the usual cap lifted.
            self.progress.xp = self.progress.threshold;
            progress::apply_levels(&mut self.progress, &mut self.player, &mut self.rng, 0, true);
            // The first four lines each pause for a keypress.
            ...
            // This line has no pause for a keypress after it.
            term::println(last);
            // Shows the character sheet, pauses for a keypress, then runs
            // the marquee.
            self.show_stats();
            term::read_key(lines);
        ...
        // The kind-3 fake-out: the same forced level-up as above, two lines,
        // and then it FALLS THROUGH into the ordinary post-fight tail
        // (spoils, item roll, den discovery) -- only the XP award and the
        // "too weak" message are skipped.
        ...
        if opponent_kind == 3 {
            ...
            // The ordinary `^2Враг сдох.` line is skipped for both rector
            // fights.
            term::println("^2Враг сдох.");
        }
        // Both rector fights skip the award line and the actual XP add.
        let award = if boss {
            0
        } else {
        ...
        // The XP award is added first, and only afterward does the game check
        // it against the level-up threshold; the two "not enough" lines print
        // only when the raw numbers fall short. That check must be done on
        // the numbers themselves, not on whether a level-up actually
        // happened: at max level no level-up is ever granted, but the
        // original still skips these lines, because the xp number alone still
        ...
        // The same rector-fight check also gates the "too weak an opponent"
        // lines.
        ...
        // The den errand's reward is the LAST thing this function does,
        // after spoils have already been claimed: `district * 20`
        // понтовость and `district * 10` xp. Applying the xp award can
        // trigger a capped level-up, which spends random draws -- so this
        // step is not text-only.
        if opponent_kind == 6 {
            let district = i32::from(self.district);
        ...
        // No district promotion happens here: a level won in this fight
        // promotes the district on the FOLLOWING turn instead, at the top of
        // the turn loop ([`Game::district_advance`]) -- and only one district
        // can be gained per turn.
        Ok(())
    }

    /// The chapter-5 endgame arm's flag stores and its three announcement
    /// lines run from [`Game::district_advance`], at the top of the turn
    /// loop.
    ///
    /// These three lines and the flag store run exactly once -- only on the
    /// turn the player is first promoted to district 5. In order: `^1Пора
    /// наконец отомстить ректору...`, a pause for a keypress, `^1Ты пробрался
    /// в универ...`, then `^1А вот и он...`, and finally the
    /// `rector_showdown` flag is set.
    ///
    /// This is the per-turn trigger, reached only once play is already under
    /// way. A second site also arms `rector_showdown`, once, at game entry
    /// (a new character or a loaded save) when the district is already 5 --
    /// ported in [`Game::apply_class_bonus`] instead.
    ///
    /// The pause for a keypress here blocks but its return value is unused;
    /// this port substitutes consuming and discarding one line of input, the
    /// same substitution `src/persist.rs`'s `choose_slot` uses elsewhere for
    /// a blocking keypress.
    ///
    /// The four calls that follow are ported in [`Game::rector_endgame`]: two
    /// calls fill a scripted enemy record with one of two fixed stat blocks,
    /// including the derived hp and damage formulas [`Game::roll_enemy`]
    /// already computes the same way, interleaved with the two rector fights
    /// themselves.
    ///
    /// This announcement runs once; the fight offer itself runs every turn
    /// instead, since nothing ever clears the flag it checks -- a player who
    /// flees both fights meets them again on the next turn.
    fn enter_district_5(&mut self, lines: &mut dyn Iterator<Item = io::Result<String>>) {
        term::println("^1Пора наконец отомстить ректору...");
        // A blocking keypress whose value is never used afterward.
        // `lines.next()` is this port's stand-in; `None` at EOF is treated
        // the same as any other discarded keystroke.
        term::read_key(lines);
    ...
    /// The two rector fights are the pair of calls that makes the game
    /// finishable.
    ///
    /// Once `rector_showdown` is set: the Den flag is set (idempotently),
    /// then the enemy record is filled and the first rector fight runs, then
    /// the enemy record is filled again and the second, harder fight runs.
    ///
    /// The enemy record for each fight is a scripted constructor, not a
    /// random roll: class 10 for both, plus its own stat block, then the
    /// four derived fields (`dmg_min = str/2`, `dmg_max = str`,
    /// `hpmax = 5*vit + str + 10`, `hp = hpmax`), with the break flags and
    /// spoils cleared. [`Enemy::to_fighter`] builds the record and
    /// [`Fighter::default`] supplies the cleared flags and spoils.
    ///
    /// **It repeats.** Nothing ever clears the `rector_showdown` flag, so a
    /// player who flees both fights meets them again on the next turn. The
    /// only exits are death (which halts the game) and beating the second
    /// rector (whose marquee halts the game).
    fn rector_endgame(
        &mut self,
        lines: &mut dyn Iterator<Item = io::Result<String>>,
    ) -> io::Result<()> {
    ...
    /// The scripted stat block for a rector fight, looked up by id.
    ///
    /// Panics on an id the table has no scripted row for. That is not a
    /// reachable branch: the two ids used are the only callers, and both
    /// are guaranteed to carry stats. A silent fallback here would turn a
    /// missing table row into a fight against a zeroed enemy, which is
    ...
    /// The hospital rescue turns a death into a survivable turn. Returns `true` when
    /// it fires -- the player lives and the fight ends.
    ///
    /// Two gates: the den must already be known, and street cred must be at least 10;
    /// otherwise it's an ordinary death and the end screen follows.
    ///
    /// Prints `^1Тебе повезло знакомые пацаны отвезли тебя в больницу` ..., then:
    /// street cred drops by 10, hp is restored to full, and a medical bill of
    /// `Round(hpmax * 3 / 5)` is subtracted from money. A broken jaw or leg subtracts
    /// a further 7 from money and clears both fractures. If money ends up negative,
    /// the deficit is paid out of street cred and money is reset to 0.
    ///
    /// Rounding is half-away-from-zero -- [`Self::round_half`]. It is unambiguous
    /// here: `hpmax * 3 / 5` is never exactly a half-integer.
    ///
    /// No random draw happens during the hospital rescue.
    fn hospital_rescue(&mut self) -> bool {
        ...
        self.player.hp = self.player.hpmax;
        // A broken jaw OR a broken leg triggers this block -- either one is
        // enough, not just both together.
        if self.player.broken_jaw || self.player.broken_leg {
            self.player.money = self.player.money.wrapping_sub(7_i16);
            self.player.broken_jaw = false;
            self.player.broken_leg = false;
        }
        // A purse driven negative by the bill is settled out of street
        // cred; the test is strict, so an exact zero does not trigger it.
    ...
    /// The `run` (flee) attempt. Returns `true` when the fight is actually
    /// left.
    ///
    /// Two refusals leave the fight running; either way, every check after
    /// `run` in the chain still runs afterward.
    ///
    /// Fleeing never consumes a random draw.
    fn flee(&mut self) -> bool {
        ...
        // A broken leg also refuses the flee attempt.
        ...
        if self.player.level > 0 {
            ...
            // At level 0 there is nothing to take; a message says so.
            term::println("^4Враг: Засранец!");
        }
        true
    }

    /// The flee penalty gives back one level.
    ///
    /// The demotion does not replay growth history backwards -- it steps
    /// forward through its own table of decrement messages, such as `^4Сила
    /// -1 `. [`crate::progress::undo_growth`] carries that table and
    /// [`crate::progress::demote`] the steps after it.
    ///
    /// The district/discovery portion of the penalty lives here rather than
    /// in `crate::progress`, since it reads the district and the discovery
    /// flags:
    /// * Class 5 skips it entirely.
    /// * The den-discovery check here tests `level - (district - 1) * 10` for
    ///   EQUALITY to 3. Intentional asymmetry: the equivalent check after
    ///   killing an enemy ([`Game::claim_spoils`]) instead uses `less than`
    ///   on the same expression.
    /// * On a match, the den flag is set and `^4Такого конявого непустят в
    ///   местный притон!` announces that the player is now too shabby for the
    ///   den, granting access. Intentional, reproduced as written.
    ///
    /// Unlike the post-kill version, there is no "already discovered" gate
    /// here, so both the flag set and the line can repeat on a second flee at
    /// the same measured level.
    fn flee_penalty(&mut self) {
        // This prints with no newline, so the stat lines below continue on
        // the same line.
        ...
        for stat in progress::undo_growth(&mut self.progress, &mut self.player) {
            term::print(match stat {
                // All four of these print with no newline too.
        ...
        // A final blank line closes the line the writes above left open.
        term::println("");
    ...
    /// The `v` (call backup) command.
    ///
    /// The call itself and the status line are two separate blocks: even a
    /// refused call still gets a countdown line if a countdown is already
    /// running. [`crate::combat_dispatch::Backup`] carries both.
    ///
    /// Calling backup never consumes a random draw.
    fn backup_in_fight(&mut self, backup: &mut Backup) {
        match backup.call(
            ...
            // Intentional: this copy HAS the trailing dot, unlike
            // the earlier one.
            ...
            // Calling backup itself prints nothing; the status line below is
            // what the player sees.
            Called::OnTheWay => {}
        }
        match backup.status(self.has_mobile) {
            // The status line shows `3 - counter` as the turns remaining.
    ...
    /// The backup's own attack, on every prompt once they've arrived.
    /// [`crate::combat_dispatch::backup_round`] carries the arithmetic and
    /// the two random draws.
    fn backup_attacks(&mut self, backup: &mut Backup, ehp: &mut i32, enemy: &Fighter) {
        ...
        *ehp = fought.enemy_hp_after;
        // Reports the damage dealt (`hp_before - hp_now`) and what's left.
        term::println(&text::fill(
    ...
    /// `f` at the fight prompt. [`crate::combat_dispatch::fire`] carries the
    /// gates, the hit test and the damage.
    fn shoot_in_fight(&mut self, ehp: &mut i32) {
        match combat_dispatch::fire(
            ...
            // Firing with no pistol prints nothing, going straight to the
            // death test.
            Shot::NoPistol => {}
            // The game's own typo: "Тельзя" for "Нельзя".
            ...
            Shot::Hit { damage } => {
                *ehp -= i32::from(damage);
                // The reported cartridge count is AFTER this shot already
                // spent one.
                term::println(&text::fill(
    ...
    /// Everything the victory block does after the XP award, in order:
    ///
    /// * loot (beer, money, junk) is added
    /// * `hp += 5`, clamped to `hpmax`
    /// * `pontovost_street += enemy.class + 1 + enemy.level / 3`
    /// * the Den opens once `level - (district-1)*10 >= 3`
    /// * `Random(30)`; only `0` reaches the one-shot gift chain
    /// * `Random(district*25)`; `luck >= roll` AND enemy class 2 grants
    ///   `Random(3)` joints
    /// * `Random(district*40)`; `luck >= roll` unlocks a class-keyed item,
    ///   each arm with its own draw
    ///
    /// Both luck comparisons widen `luck` and the roll to `i32` from `u16`,
    /// so a `luck` value with the high bit set is never read as negative.
    fn claim_spoils(&mut self, enemy: &Fighter) {
        ...
        // The +5 is stored only while it stays at or below hpmax; above it,
        // hpmax is stored instead.
        self.player.hp = (self.player.hp + 5).min(self.player.hpmax);
        // A word add, so `pontovost_street` wraps on overflow.
        self.pontovost_street = self
            .pontovost_street
            .wrapping_add((enemy.class + 1 + enemy.level / 3) as i16);
        // The level is measured within the district, so three levels into a
        // district opens the den.
        if !self.places.is_found(Location::Den)
            && i32::from(self.player.level) - (i32::from(self.district) - 1) * 10 >= 3
        {
            self.places.mark_found(Location::Den);
        ...
        // Only a 0 out of `Random(30)` reaches the gift chain.
        ...
        match enemy.class {
            ...
            9 => self.spoil_blade(),
            // Classes 0, 2 and 8 (and anything above 9) get no item at all --
            // the chain names no arm for them.
            _ => {}
        }
    }

    /// The first one-shot gift that has not fired yet, on `Random(30) == 0`.
    ///
    /// The same three grants as `Game::church`'s arm 2.
    ///
    /// The preamble line is printed when ANY of the three is still unfired.
    fn grant_oneshot_gift(&mut self) {
        ...
        // This chain is a separate if/else on the same three flags -- only
        // the first still-unfired gift grants, unlike the preamble's check
        // above, which is an OR across all three.
        if !self.oneshot_gift_1 {
    ...
    fn spoil_club(&mut self) {
        match self.rng.below(2) {
            0 => {
                ...
                self.weapon_kastet = true;
                term::println(spoils::EMITTED[11].1);
                // The "better" set here: ножик, дубинка, тесак.
            ...
            _ => {
                ...
                self.weapon_dubinka = true;
                term::println(spoils::EMITTED[13].1);
                // The "better" set here: ножик, тесак.
                if self.weapon_nozhik || self.weapon_tesak {
    ...
    /// Enemy class 7 (Беспредельщик): `Random(2)`.
    ...
    /// Enemy class 9 (Маньячок): `Random(2)`.
    ///
    /// The damage terms are a chain of independent `if`s, not a `match`:
    /// each arm can add more than one of them.
    ...
    /// The crowd that gathers around a long fight, and the two draws it
    /// spends.
    ///
    /// The prompt counter stops at 5, so from the fifth `Битва\` prompt
    /// onward, `Random(10)` fires at every prompt, not once.
    ///
    /// Called BEFORE the prompt is written.
    fn crowd(&mut self, prompts_seen: &mut u8) {
        ...
        // The rector showdown suppresses only the taunts (and their two
        // draws) -- the earlier `^7Начинают собираться зрители` line still
        // prints. [`Game::enter_district_5`] sets [`Game::rector_showdown`]
        ...
        // Only a 0 out of `Random(10)` reaches the taunt draw.
        if self.rng.below(10) != 0 {
            return;
        }
        let which = self.rng.below(18);
        // Two of the eighteen spectator lines splice in a name: one uses
        // the player's rank, the other the player's own name.
        match which {
    ...
    /// The rank name, looked up from the same per-class table that supplies
    /// the enemy's display name (one row per class).
    ...
    ///
    /// Per-blow messages: miss (`^4Ты промазал`, `^2Враг промазал`), hit
    /// (`^2Ты пнул врага на #з. У него осталось #` and its mirror `^4Он пнул
    /// тебя на #з. У тебя осталось #`), and break (`^2Ты сломал врагу
    /// челюсть. ^4Враг: А! козёл!` / `^2Ты сломал врагу ногу. ^4Враг: Ну что
    /// за урод!`, whose inner `^4` is part of the string, and the mirrors
    /// `^4Враг сломал тебе челюсть.` / `^4Враг сломал тебе ногу.`).
    ///
    /// The `Random(3)` crit pick and the зубная защита both choose between
    /// multiple lines:
    ///
    /// * the player's crit trio -- `^2Точный удар!!!`, `^2Не хило
    ///   приложил!!!`, `^2Двойной урон!!!`;
    /// * the enemy's crit trio -- `^4Враг:Сдохни урод!!`, `^4Тебе не хило
    ///   врезали!`, `^4Враг:Получи гнида!!`;
    /// * the two зубная защита arms `Random(4)` picks between --
    ///   `^4Враг сломал тебе челюсть, даже защита не помогла.` on a 0, and
    ///   `^2Защита спасла твои кривые клыки.` otherwise;
    /// * the two "ещё раз" lines, `^2Из-за большой ловкости ты можешь пнуть` ...
    ///   and its enemy mirror, `^4Из-за большой ловкости враг может пнуть
    ///   ещё раз`, whose guards are NOT mirror images -- see the comment on
    ///   the enemy loop's tail below.
    fn combat_round(&mut self, enemy: &mut Fighter) {
        // The two loops exit differently: the player's loop stops once the
        // enemy's hp is <= 0, but the enemy's loop only stops once the
        // player's hp is < 0 -- so a defender sitting at EXACTLY 0 stops the
        // player's loop but not the enemy's, and the enemy swings again,
        // costing another draw. `Fighter::hp` is a `u16` that saturates at
        // 0, which cannot tell "exactly 0" from "would have gone negative",
        // so the running hp is kept here as an `i32` and the loop exits are
        // driven from it; only the stored value saturates.
        let mut ehp = i32::from(enemy.hp);
        ...
        for i in 0..player_blows {
            ...
            // The break message only prints once per limb -- suppressed
            // once that limb is already broken.
            match blow.broke {
                ...
                // Already broken, or no break at all: no message, flag
                // unchanged.
                _ => {}
            }
            // No "ещё раз" message when the enemy is already down; it only
            // prints when the blow budget still has room for another swing.
...
pub const COMMAND_LIST: [(bool, &str); 17] = [
    ...
    (true, "Напиши: ^6mar^7  чтобы идти на рынок"),
    (true, "Напиши: ^6bmar^7 чтобы идти к барыгам"),
    (true, "Напиши: ^6rep^7  чтобы идти к ветеринару"),
    (true, "Напиши: ^6girl^7 чтобы завалиться к своей девчонке"),
    (true, "Напиши: ^6pr^7   чтобы идти в местный притон гопоты"),
    (true, "Напиши: ^6kl^7   чтобы идти в клуб"),
    (true, "Напиши: ^6trn^7  чтобы идти в качалку"),
    (
        true,
        "Напиши: ^6s^7    чтобы посмотреть в лужу на свою уродскую рожу",
    ),
    ...
    (true, "Напиши: ^6v^7    чтобы позвать подкрепление"),
    (true, "Напиши: ^6kos^7  чтобы схавать косяк"),
    (
        true,
        "Напиши: ^6h^7    чтобы выпить пиво (если не охото к ветеринару)",
    ),
    (true, "Напиши: ^6mh^7   чтобы набухаться до чёртиков"),
    (true, "Напиши: ^6name^7 чтобы сменить погоняло"),
    (true, "Напиши: ^6e^7    если захочешь выйти"),
];
...
mod tests {
    ...
    fn the_class_keyed_opener_runs_only_for_param_1_zero_and_six() {
        let greeted = |kind: u8| {
            let mut g = game();
            g.player.level = 0; // `run` leaves with no penalty.
            term::capture::lines(|| {
    ...
    fn shop_mode_leaves_on_w_and_ignores_other_verbs() {
        ...
        g.player.hp = 1; // Ejects a whole player after every turn
    ...
    fn h_heals_at_the_vet_and_drinks_beer_on_the_street() {
        ...
        g.location = Location::Vet;
        g.mode = Mode::Shop(Location::Vet);
        g.player.hp = g.player.hpmax - 6; // Needs hp < hpmax
        g.player.money = 10;
        g.player.beer_dl = 4;
        let hp0 = g.player.hp;
    ...
    fn the_silencer_row_is_listed_only_with_a_pistol_and_a_full_counter() {
        let listed = |pistol: bool, counter: u8| {
            let mut g = game();
            g.district = 4; // Satisfied throughout
    ...
    fn the_market_boots_end_on_two_damage_in_either_purchase_order() {
        ...
        let mut g = market(100);
        g.district = 3; // Is `cmp byte [0x3692],0x2`
    ...
    fn the_market_jackets_end_on_four_armour_in_either_purchase_order() {
        let mut g = market(100);
        g.district = 4; // Is `cmp byte [0x3692],0x3`
        g.shop_turn(Location::Market, "6", &mut no_input()).unwrap();
        assert!(g.wear_jacket, "1000:c0e0");
    ...
    fn all_sellable(g: &mut Game) {
        ...
        g.wear_jacket_krutaya = true;
        g.weapon_kastet = true;
        g.weapon_dubinka = true;
        g.weapon_nozhik = true;
        g.weapon_tesak = true;
    }
    ...
    fn trained_armour_subtracts_what_the_equipment_granted() {
        ...
        // One of each, alone.
        assert_eq!(abs(10, true, false, false, false), 9);
        assert_eq!(abs(10, false, true, false, false), 8);
        assert_eq!(abs(10, false, false, true, false), 8);
        assert_eq!(abs(10, false, false, false, true), 6);
    ...
    fn walk_tail(phone: bool) -> Vec<(u16, Option<u16>)> {
        let mut v = Vec::new();
        if phone {
            v.push((200, None)); // The wrong-number gag
            v.push((100, None)); // The girl's call
        }
    ...
    fn the_den_menu_prints_all_twelve_lines_when_every_gate_is_open() {
        ...
        assert_eq!(
            ...
            vec![
                "",
                "^6На одного пацана наехал какой-то урод",
                "^6Ты пацан нормальный. Есть дело.",
                "^6Пацаны хотят тебе кое-чё сказать",
                "",
                ...
                "Напиши ^6a^7  чтобы спросить чё-то",
                "Напиши ^6d^7 чтобы пойти на дело",
            ]
        );
    }
    ...
    fn the_walk_preamble_counters_tick_once_each() {
        ...
        for (found, pistol, phone, start, want, prints) in [
            ...
            (true, true, true, 25, 25, false), // The < 25 gate
            (false, true, true, 0, 0, false),  // Dealers unknown
            (true, false, true, 0, 0, false),  // No pistol
        ] {
            let mut g = quiet_walker();
            if found {
                g.places.mark_found(Location::Dealers);
    ...
    fn the_walk_preamble_ring_and_class_perks_move_hp() {
        ...
        // The Вор. Draw 10 is `Random(district * 20)` and the theft succeeds
        // on `luck >= r`; draw 11 is `Random(district * 5)` and the take is
        // `r + 1`. A tie (`luck == r`) still succeeds, since the check is
        // `>=`, not `>`.
    ...
    /// starting weapon set (`spoil_club` and `spoil_blade`).
    ///
    /// Each expected delta is summed per arm by the conditions above them,
    /// which are chained `if`s, not a `match`, so one arm can add several.
    ///
    /// Two rows look wrong and are not, and both are intentional: a ножик
    /// found by a дубинка owner adds 6 (4 plus 2), and a тесак found by a
    /// дубинка owner adds 12 (7 plus 5). Both are what the chains compute;
    /// the port transliterates them rather than rationalising.
    #[test]
    fn the_weapon_spoils_table_adds_exactly_these_damages() {
        // (arm draw, starting (kastet, nozhik, dubinka, tesak), delta)
        let club: [SpoilRow; 9] = [
            // Draw 0 -- the кастет, "урон+2".
            (0, (false, false, false, false), 2),
            (0, (true, false, false, false), 0), // already owned: 1000:553a
            (0, (false, true, false, false), 0), // Better weapon
            (0, (false, false, true, false), 0),
            (0, (false, false, false, true), 0),
            // Draw 1 -- the дубинка, "урон+4".
    ...
    /// `spoil_charm` and `spoil_glasses` -- luck, and the two flags that
    /// carry no stat.
    #[test]
    fn the_charm_spoils_grant_luck_once_each() {
        ...
        // Flags only, no stat moves at all.
        for draw in [0u16, 1] {
            let mut g = game();
            g.rng = Rng::new(seed_drawing(&[(2, draw)]));
    ...
    /// The post-kill one-shot gift chain -- the second grant site for the
    /// same three rings `Game::church`'s arm 2 hands out.
    ///
    /// `dmg_min` starts at 3, not the fixture's 1: ring 1 leaves it at 4 and
    /// ring 2's `+= 2` would read the same as `*= 2` from 2.
    #[test]
    fn the_post_kill_gift_chain_grants_each_ring_once_in_order() {
        ...
        // its parity test fires; ring 2's `+= 2` is unconditional.
        let want: [[i64; 8]; 3] = [
            [6, 6, 1, 1, 1, 1, 1, 1],
            [30, 30, 5, 5, 5, 5, 3, 5],
            [30, 30, 5, 5, 5, 5, 3, 5],
        ...
        for (round, want) in want.iter().enumerate() {
            ...
            // The header is gated on its own three-way OR, separate from
            // the if/else chain below it, so it still prints on the round
            // that hands out the last ring.
            assert!(
    ...
    /// Every stat the church's draw-15 table moves, arm by arm -- a
    /// `Random(5)` roll and, inside its `1` arm, a `Random(4)` roll.
    ///
    /// The deltas touch strength, agility, vitality, luck, the two rings,
    /// armour, and понтовость.
    ///
    /// **Both strength parities are driven**, because the parity check reads
    /// the ALREADY-incremented strength: starting even leaves it odd and
    /// `dmg_min` still, starting odd leaves it even and `dmg_min` moves.
    #[test]
    fn the_church_award_table_moves_exactly_these_stats() {
        ...
        /// The fields the church table can touch, in `src/model.rs` order.
        ...
        for (arm, sub, strength, want) in table {
            ...
            // Nothing else in the table touches armour.
            assert_eq!(
        ...
        // This arm's gain: `district * 50 + 50`, and it prints the amount.
        ...
        // text-only. `player()` starts at strength 5, so ring 1 lands it on
        // 6 (dmg_min +1) and ring 2's unconditional `+= 2` follows.
        let mut g = game();
    ...
    /// `run` is NOT a shop exit. Only `w` matches, and nothing else -- a
    /// `run` falls through to the prompt, silently. The street is the only
    /// dispatch with a second synonym (`w` AND `run`), which is why
    /// `commands::parse` folds the two, and why `shop_turn` must not use it.
    ///
    /// Every location goes through the same shared arm, so this holds for
    /// the vet, club, gym, market and dealers too; the vet's extra `e` exit
    /// is its own arm above and is unaffected.
    #[test]
    ...
    /// Reaching district 5 sets `rector_showdown`, via
    /// [`Game::enter_district_5`], and the flee refusal ([`Game::flee`]) is
    /// the reader this test drives live.
    ///
    /// Reaching district 5 also prompts to save; answering `n` here means
    /// the save arm is not taken, and `enter_district_5` then reads one more
    /// key.
    #[test]
    fn reaching_district_5_arms_the_rector_showdown_and_flee_refuses() {
        ...
        // The reader refuses `run` outright while rector_showdown is set.
        // Reusing `g` itself -- not a fresh instance with the field poked
        // directly -- is the point: this is the flag `enter_district_5` just
        // armed, changing behaviour on the very game it armed it on.
        let fled = g.flee();
        assert!(!fled, "1000:48eb refuses every flee once the flag is set");
    }

    /// A won fight no longer promotes the district by itself; the advance
    /// runs at the top of the NEXT turn.
    #[test]
    ...
    /// **One district per turn, never four.**
    #[test]
    fn the_advance_gains_at_most_one_district_per_turn() {
        ...
        g.district_advance(&mut input(&["n", ""])).unwrap();
        assert_eq!(g.district, 5);
        // A sixth advance is refused.
        g.district_advance(&mut input(&["n"])).unwrap();
        assert_eq!(g.district, 5, "1000:ab8f jumps past the increment");
    }

    /// Both gates refuse before anything is read, so the line the street
    /// prompt is about to read must still be there.
    #[test]
    ...
    /// Both ban countdowns are cleared on the advance itself -- not by the
    /// save arm, which the `n` here declines.
    #[test]
    ...
    /// The market is open only while the countdown is zero, and a non-zero
    /// one leads to a refusal that neither enters nor changes anything.
    #[test]
    ...
    /// The discovery check runs before the ban check, so an undiscovered
    /// market prints its own refusal whatever the countdown says. Testing
    /// the ban first would print the other line.
    #[test]
    ...
    /// The `girl` block heals, charges 12 and clears the market ban.
    #[test]
    ...
    /// preamble: set to 5 ([`crate::market`]), ticked down once per walk,
    /// announced on its last turn, and gone on the fifth. The `== 1` test
    /// runs BEFORE the tick, so the message and the last tick share a turn.
    #[test]
    ...
    /// `^2Враг сдох.`, no XP award line and no "too weak an opponent" pair.
    #[test]
    fn a_boss_kill_prints_none_of_the_ordinary_victory_lines() {
        ...
        // Boss XP is set to the threshold and spent with the level cap
        // LIFTED -- the test player starts at MAX_LEVEL, so a capped call
        // would move nothing.
        assert_eq!(g.player.level, before_level + 1, "1000:5094 passes 1");
        assert_eq!(before_level, crate::progress::MAX_LEVEL);
    }

    /// The two stat blocks the boss constructor writes -- the constants,
    ...
    /// The den errand's reward: `district * 20` понтовость and
    /// `district * 10` xp, on top of the ordinary combat award.
    ///
    ...
    /// the level drain it feeds. The control run uses the same starting
    /// state and script, so the difference between the two is just the
    /// errand.
    #[test]
    fn the_den_errand_pays_cred_and_experience_and_nothing_else_does() {
        ...
        // `district * 10`, on top of the ordinary award the control also
        // gets.
        ...
        // Every other errand kind skips it.
    ...
    /// The three openers the `0 | 6` gate does NOT cover, and the silence
    /// of everything else.
    #[test]
    fn each_param_value_greets_with_its_own_opener() {
        ...
        // 2 and 5 reach no opener at all: none of the chain's three
        // compares names them.
        for kind in [2u8, 5] {
            let mut g = game();
            let out = term::capture::lines(|| {
    ...
    /// Both deaths run the end screen -- the rector's and the plain one --
    /// each behind its own key press.
    #[test]
    fn dying_draws_the_end_screen() {
        for rector in [false, true] {
            ...
            // The death banner's colour digit is '4'.
            assert!(
    ...
    // -----------------------------------------------------------------
    // The opening and the text dumps.
    // -----------------------------------------------------------------

    /// `help`'s only branch: one line appears from district 2 on and from
    /// nowhere else. Both sides are asserted, so neither can pass by
    /// printing everything or nothing.
    #[test]
    ...
    /// reason `help` is not a static blob: two append the class's rank
    /// name, one the player's name, and one pushes the class's STRENGTH
    /// growth weight into both filled lines.
    #[test]
    ...
    /// `e` / `exit` at the STREET prompt prints two lines, then the FULL
    /// character sheet, then reads one discarded key.
    #[test]
    ...
    /// This chain keys on the DISTRICT, and adds the tutorial for district
    /// 1 only. Districts 2, 3 and 4 print the same wording as
    /// `Game::district_advance`, from the OTHER string copy.
    #[test]
    ...
    /// This chain has arms for 2, 3 and 4 only, and it reads the district
    /// AFTER the increment -- so the line the player sees names the
    /// district they arrived in, not the one they left.
    #[test]
